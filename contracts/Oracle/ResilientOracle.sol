...
contract ResilientOracle is PausableUpgradeable, AccessControlledV8, ResilientOracleInterface {
    ...
    address public immutable vCore;

    /// @notice UAI address
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    address public immutable uai;
    ...
    /// @param vCoreAddress The address of the vCORE
    /// @param uaiAddress The address of the UAI
    /// @param _boundValidator Address of the bound validator contract
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(
        address vCoreAddress,
        address uaiAddress,
        BoundValidatorInterface _boundValidator
    ) notNullAddress(vCoreAddress) notNullAddress(uaiAddress) notNullAddress(address(_boundValidator)) {
        vCore = vCoreAddress;
        uai = uaiAddress;
    ...
    function _getUnderlyingAsset(address vToken) private view returns (address asset) {
        if (address(vToken) == vCore) {
            asset = CORE_ADDR;
        } else if (address(vToken) == uai) {
            asset = uai;
        } else {
