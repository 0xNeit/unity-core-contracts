 ...
 * @title TwapOracle
 * @author Unity Core
 * @notice This oracle fetches price of assets from IceCreamSwap.
 */
contract TwapOracleIce is AccessControlledV8, TwapInterface {
    ...
    struct TokenConfig {
        ...
        uint256 baseUnit;
        /// @notice The address of IceCream pair
        address icecreamPool;
        /// @notice Whether the token is paired with WCORE
        bool isCoreBased;
        /// @notice A flag identifies whether the IceCream pair is reversed
        /// e.g. UCORE-WCORE is reversed, while WCORE-UCORE is not.
        bool isReversedPool;
        /// @notice The minimum window in seconds required between TWAP updates
        uint256 anchorPeriod;
    }

    /// @notice Set this as asset address for CORE. This is the underlying for vCORE
    address public constant CORE_ADDR = 0x40375C92d9FAf44d2f9db9Bd9ba41a3317a2404f;

    /// @notice WCORE address
    /// @custom:oz-upgrades-unsafe-allow state-variable-immutable
    address public immutable WCORE;

    /// @notice the base unit of WCORE and USDT, which are the paired tokens for all assets
    uint256 public constant CORE_BASE_UNIT = 1e18;
    uint256 public constant USDT_BASE_UNIT = 1e18;
    ...
    event TokenConfigAdded(address indexed asset, address indexed icecreamPool, uint256 indexed anchorPeriod);
    ...
    /// @param wCoreAddress The address of the WCORE
    /// @custom:oz-upgrades-unsafe-allow constructor
    constructor(address wCoreAddress) notNullAddress(wCoreAddress) {
        WCORE = wCoreAddress;
     ...
     * @custom:error NotNullAddress error is thrown if IceCreamSwap pool address is null
     * @custom:event Emits TokenConfigAdded event if new token config are added with
     * asset address, IceCreamPool address, anchor period address
     */
    function setTokenConfig(
        TokenConfig memory config
    ) public notNullAddress(config.asset) notNullAddress(config.icecreamPool) {
        ...
        emit TokenConfigAdded(config.asset, config.icecreamPool, config.anchorPeriod);
    }

    /**
     * @notice Updates the current token/USDT price from IceCreamSwap, with 18 decimals of precision.
    ...
    function updateTwap(address asset) public returns (uint256) {
        ...
        if (tokenConfigs[asset].asset == address(0)) revert("asset not exist");
        // Update & fetch WCORE price first, so we can calculate the price of WCORE paired token
        if (asset != WCORE && tokenConfigs[asset].isCoreBased) {
            if (tokenConfigs[WCORE].asset == address(0)) revert("WCORE not exist");
            _updateTwapInternal(tokenConfigs[WCORE]);
        }
        return _updateTwapInternal(tokenConfigs[asset]);
    }

    /**
     * @notice Fetches the current token/WCORE and token/USDT price accumulator from IceCreamSwap.
    ...
    function currentCumulativePrice(TokenConfig memory config) public view returns (uint256) {
        (uint256 price0, uint256 price1, ) = OracleLibrary.currentCumulativePrices(config.icecreamPool);
    ...
    /**
     * @notice Fetches the current token/USDT price from IceCreamSwap, with 18 decimals of precision.
     ...
     * @custom:error Zero price error is thrown if token is CORE based and price is zero
    ...
    function _updateTwapInternal(TokenConfig memory config) private returns (uint256) {
        ...
        // priceAverage will always be Token/CORE or Token/USDT *twap** price.
        ...
        // tokenBaseUnit / (CORE_BASE_UNIT or USDT_BASE_UNIT, which is 1e18)
        uint256 pairedTokenBaseUnit = config.isCoreBased ? CORE_BASE_UNIT : USDT_BASE_UNIT;
        uint256 anchorPriceMantissa = (priceAverageMantissa * config.baseUnit) / pairedTokenBaseUnit;

        // if this token is paired with CORE, convert its price to USD
        if (config.isCoreBased) {
            uint256 corePrice = prices[WCORE];
            if (corePrice == 0) revert("core price is invalid");
            anchorPriceMantissa = (anchorPriceMantissa * corePrice) / CORE_BASE_UNIT;
        }
