 ...
 * @title TwapOracle
 * @author Unity Core
 * @notice This oracle fetches price of assets from ArcherSwap.
 */
contract TwapOracleArcher is AccessControlledV8, TwapInterface {
    ...
    struct TokenConfig {
        ...
        uint256 baseUnit;
        /// @notice The address of Archer pair
        address archerPool;
        /// @notice Whether the token is paired with WCORE
        bool isCoreBased;
        /// @notice A flag identifies whether the Archer pair is reversed
    ...
    event TokenConfigAdded(address indexed asset, address indexed archerPool, uint256 indexed anchorPeriod);
     ...
     * @custom:error NotNullAddress error is thrown if ArcherSwap pool address is null
     * @custom:event Emits TokenConfigAdded event if new token config are added with
     * asset address, ArcherPool address, anchor period address
     */
    function setTokenConfig(
        TokenConfig memory config
    ) public notNullAddress(config.asset) notNullAddress(config.archerPool) {
        ...
        emit TokenConfigAdded(config.asset, config.archerPool, config.anchorPeriod);
    }

    /**
     * @notice Updates the current token/USDT price from ArcherSwap, with 18 decimals of precision.
    ...
    /**
     * @notice Fetches the current token/WCORE and token/USDT price accumulator from ArcherSwap.
    ...
    function currentCumulativePrice(TokenConfig memory config) public view returns (uint256) {
        (uint256 price0, uint256 price1, ) = OracleLibrary.currentCumulativePrices(config.archerPool);
    ...
    /**
     * @notice Fetches the current token/USDT price from ArcherSwap, with 18 decimals of precision.
