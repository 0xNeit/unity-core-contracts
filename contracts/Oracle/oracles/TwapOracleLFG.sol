 ...
 * @title TwapOracle
 * @author Unity Core
 * @notice This oracle fetches price of assets from LFGSwap.
 */
contract TwapOracleLFG is AccessControlledV8, TwapInterface {
    ...
    struct TokenConfig {
        ...
        uint256 baseUnit;
        /// @notice The address of LFG pair
        address lfgPool;
        /// @notice Whether the token is paired with WCORE
        bool isCoreBased;
        /// @notice A flag identifies whether the LFG pair is reversed
    ...
    event TokenConfigAdded(address indexed asset, address indexed lfgPool, uint256 indexed anchorPeriod);
     ...
     * @custom:error NotNullAddress error is thrown if LFGSwap pool address is null
     * @custom:event Emits TokenConfigAdded event if new token config are added with
     * asset address, LFGPool address, anchor period address
     */
    function setTokenConfig(
        TokenConfig memory config
    ) public notNullAddress(config.asset) notNullAddress(config.lfgPool) {
        ...
        emit TokenConfigAdded(config.asset, config.lfgPool, config.anchorPeriod);
    }

    /**
     * @notice Updates the current token/USDT price from LFGSwap, with 18 decimals of precision.
    ...
    /**
     * @notice Fetches the current token/WCORE and token/USDT price accumulator from LFGSwap.
    ...
    function currentCumulativePrice(TokenConfig memory config) public view returns (uint256) {
        (uint256 price0, uint256 price1, ) = OracleLibrary.currentCumulativePrices(config.lfgPool);
    ...
    /**
     * @notice Fetches the current token/USDT price from LFGSwap, with 18 decimals of precision.
