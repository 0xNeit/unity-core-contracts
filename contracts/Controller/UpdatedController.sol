    ...
    ExponentialNoError
{
    ...
    function _become(Unitroller unitroller) external {
        ...
        UpdatedController(address(unitroller))._upgradeSplitUcoreRewards();
    }

    function _upgradeSplitUcoreRewards() external {
        require(msg.sender == controllerImplementation, "only brains can become itself");

        uint32 blockNumber = safe32(getBlockNumber(), "block number exceeds 32 bits");

        // ucoreSpeeds -> ucoreBorrowSpeeds & ucoreSupplySpeeds t
        for (uint256 i; i < allMarkets.length; ++i) {
            ucoreBorrowSpeeds[address(allMarkets[i])] = ucoreSupplySpeeds[address(allMarkets[i])] = ucoreSpeeds[
                address(allMarkets[i])
            ];
            delete ucoreSpeeds[address(allMarkets[i])];

            /*
             * Ensure supply and borrow state indices are all set. If not set, update to default value
             */
            UcoreMarketState storage supplyState = ucoreSupplyState[address(allMarkets[i])];
            UcoreMarketState storage borrowState = ucoreBorrowState[address(allMarkets[i])];
